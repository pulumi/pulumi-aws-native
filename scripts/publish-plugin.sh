...
PLUGIN_PACKAGE_NAME="pulumi-resource-cloudformation-${VERSION}-$(go env GOOS)-$(go env GOARCH).tar.gz"
...
go build \
   -ldflags "-X github.com/pulumi/pulumi-cloudformation/pkg/version.Version=${VERSION}" \
   -o "${WORK_PATH}/pulumi-resource-cloudformation${BIN_SUFFIX}" \
   "${ROOT}/cmd/pulumi-resource-cloudformation"
...
CREDS_JSON=$(aws sts assume-role \
                 ...
                 --role-session-name "upload-plugin-pulumi-resource-cloudformation" \
