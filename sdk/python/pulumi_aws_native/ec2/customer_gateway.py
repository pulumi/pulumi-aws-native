...
class CustomerGatewayArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[_builtins.str] ip_address: IPv4 address for the customer gateway device's outside interface. The address must be static. If ``OutsideIpAddressType`` in your VPN connection options is set to ``PrivateIpv4``, you can use an RFC6598 or RFC1918 private IPv4 address. If ``OutsideIpAddressType`` is set to ``PublicIpv4``, you can use a public IPv4 address.
        ...
        :param pulumi.Input[_builtins.float] bgp_asn_extended: For customer gateway devices that support BGP, specify the device's ASN. You must specify either ``BgpAsn`` or ``BgpAsnExtended`` when creating the customer gateway. If the ASN is larger than ``2,147,483,647``, you must use ``BgpAsnExtended``.
...
class CustomerGateway(pulumi.CustomResource):
    ...
    @_builtins.property
    @pulumi.getter(name="bgpAsnExtended")
    def bgp_asn_extended(self) -> pulumi.Output[Optional[_builtins.float]]:
        """
        For customer gateway devices that support BGP, specify the device's ASN. You must specify either ``BgpAsn`` or ``BgpAsnExtended`` when creating the customer gateway. If the ASN is larger than ``2,147,483,647``, you must use ``BgpAsnExtended``.
         Valid values: ``2,147,483,648`` to ``4,294,967,295``
        """
