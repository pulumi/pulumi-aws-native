...
warnings.warn("""DirectoryConfig is not yet supported by AWS Cloud Control API, so its creation will currently fail. Please use the classic AWS provider, if possible.""", DeprecationWarning)
...
class DirectoryConfig(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
                 ...
                 service_account_credentials: Optional[pulumi.Input[pulumi.InputType['DirectoryConfigServiceAccountCredentialsArgs']]] = None,
                 __props__=None):
        pulumi.log.warn("""DirectoryConfig is deprecated: DirectoryConfig is not yet supported by AWS Cloud Control API, so its creation will currently fail. Please use the classic AWS provider, if possible.""")
        if opts is None:
