...
class FirewallPolicyStatefulRuleGroupReferenceArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[_builtins.int] priority: An integer setting that indicates the order in which to run the stateful rule groups in a single `FirewallPolicy` . This setting only applies to firewall policies that specify the `STRICT_ORDER` rule order in the stateful engine options settings.
