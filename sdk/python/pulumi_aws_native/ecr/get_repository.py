...
class GetRepositoryResult:
    def __init__(__self__, arn=None, image_scanning_configuration=None, image_tag_mutability=None, image_tag_mutability_exclusion_filters=None, lifecycle_policy=None, repository_policy_text=None, repository_uri=None, tags=None):
...
def get_repository(repository_name: Optional[builtins.str] = None,
    ...
    :param builtins.str repository_name: The name to use for the repository. The repository name may be specified on its own (such as ``nginx-web-app``) or it can be prepended with a namespace to group the repository into a category (such as ``project-a/nginx-web-app``). If you don't specify a name, CFNlong generates a unique physical ID and uses that ID for the repository name. For more information, see [Name type](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/aws-properties-name.html).
