...
class ZonalAutoshiftConfigurationPracticeRunConfigurationArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[Sequence[pulumi.Input['ZonalAutoshiftConfigurationControlConditionArgs']]] blocking_alarms: A control condition is an alarm that you specify for a practice run. When you configure practice runs with zonal autoshift for a resource, you specify Amazon CloudWatch alarms, which you create in CloudWatch to use with the practice run. The alarms that you specify are an *outcome alarm* , to monitor application health during practice runs and, optionally, a *blocking alarm* , to block practice runs from starting or to interrupt a practice run in progress.
