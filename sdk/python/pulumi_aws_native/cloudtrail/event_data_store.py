...
warnings.warn("""EventDataStore is not yet supported by AWS Native, so its creation will currently fail. Please use the classic AWS provider, if possible.""", DeprecationWarning)
...
class EventDataStore(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
                 ...
                 termination_protection_enabled: Optional[pulumi.Input[bool]] = None,
                 __props__=None):
        pulumi.log.warn("""EventDataStore is deprecated: EventDataStore is not yet supported by AWS Native, so its creation will currently fail. Please use the classic AWS provider, if possible.""")
        if opts is None:
