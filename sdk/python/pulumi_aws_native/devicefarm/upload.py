# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from ._enums import *

__all__ = ['UploadArgs', 'Upload']

@pulumi.input_type
class UploadArgs:
    def __init__(__self__, *,
                 project_arn: pulumi.Input[_builtins.str],
                 type: pulumi.Input['UploadType'],
                 content_type: pulumi.Input[Optional[_builtins.str]] = None,
                 name: pulumi.Input[Optional[_builtins.str]] = None):
        """
        The set of arguments for constructing a Upload resource.

        :param pulumi.Input[_builtins.str] project_arn: The ARN of the project for the upload.
        :param pulumi.Input['UploadType'] type: The upload's type.
        :param pulumi.Input[_builtins.str] content_type: The upload's content type (for example, application/octet-stream).
        :param pulumi.Input[_builtins.str] name: The upload's file name.
        """
        pulumi.set(__self__, "project_arn", project_arn)
        pulumi.set(__self__, "type", type)
        if content_type is not None:
            pulumi.set(__self__, "content_type", content_type)
        if name is not None:
            pulumi.set(__self__, "name", name)

    @_builtins.property
    @pulumi.getter(name="projectArn")
    def project_arn(self) -> pulumi.Input[_builtins.str]:
        """
        The ARN of the project for the upload.
        """
        return pulumi.get(self, "project_arn")

    @project_arn.setter
    def project_arn(self, value: pulumi.Input[_builtins.str]):
        pulumi.set(self, "project_arn", value)

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Input['UploadType']:
        """
        The upload's type.
        """
        return pulumi.get(self, "type")

    @type.setter
    def type(self, value: pulumi.Input['UploadType']):
        pulumi.set(self, "type", value)

    @_builtins.property
    @pulumi.getter(name="contentType")
    def content_type(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        The upload's content type (for example, application/octet-stream).
        """
        return pulumi.get(self, "content_type")

    @content_type.setter
    def content_type(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "content_type", value)

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        The upload's file name.
        """
        return pulumi.get(self, "name")

    @name.setter
    def name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "name", value)


@pulumi.type_token("aws-native:devicefarm:Upload")
class Upload(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 content_type: pulumi.Input[Optional[_builtins.str]] = None,
                 name: pulumi.Input[Optional[_builtins.str]] = None,
                 project_arn: pulumi.Input[Optional[_builtins.str]] = None,
                 type: pulumi.Input[Optional['UploadType']] = None,
                 __props__=None):
        """
        Represents an app or test scripts upload in AWS Device Farm.

        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] content_type: The upload's content type (for example, application/octet-stream).
        :param pulumi.Input[_builtins.str] name: The upload's file name.
        :param pulumi.Input[_builtins.str] project_arn: The ARN of the project for the upload.
        :param pulumi.Input['UploadType'] type: The upload's type.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: UploadArgs,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Represents an app or test scripts upload in AWS Device Farm.

        :param str resource_name: The name of the resource.
        :param UploadArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(UploadArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 content_type: pulumi.Input[Optional[_builtins.str]] = None,
                 name: pulumi.Input[Optional[_builtins.str]] = None,
                 project_arn: pulumi.Input[Optional[_builtins.str]] = None,
                 type: pulumi.Input[Optional['UploadType']] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = UploadArgs.__new__(UploadArgs)

            __props__.__dict__["content_type"] = content_type
            __props__.__dict__["name"] = name
            if project_arn is None and not opts.urn:
                raise TypeError("Missing required property 'project_arn'")
            __props__.__dict__["project_arn"] = project_arn
            if type is None and not opts.urn:
                raise TypeError("Missing required property 'type'")
            __props__.__dict__["type"] = type
            __props__.__dict__["arn"] = None
            __props__.__dict__["category"] = None
            __props__.__dict__["created"] = None
            __props__.__dict__["status"] = None
        replace_on_changes = pulumi.ResourceOptions(replace_on_changes=["contentType", "name", "projectArn", "type"])
        opts = pulumi.ResourceOptions.merge(opts, replace_on_changes)
        super(Upload, __self__).__init__(
            'aws-native:devicefarm:Upload',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None) -> 'Upload':
        """
        Get an existing Upload resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = UploadArgs.__new__(UploadArgs)

        __props__.__dict__["arn"] = None
        __props__.__dict__["category"] = None
        __props__.__dict__["content_type"] = None
        __props__.__dict__["created"] = None
        __props__.__dict__["name"] = None
        __props__.__dict__["project_arn"] = None
        __props__.__dict__["status"] = None
        __props__.__dict__["type"] = None
        return Upload(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def arn(self) -> pulumi.Output[_builtins.str]:
        """
        The Amazon Resource Name (ARN) of the upload.
        """
        return pulumi.get(self, "arn")

    @_builtins.property
    @pulumi.getter
    def category(self) -> pulumi.Output['UploadCategory']:
        """
        The upload's category.
        """
        return pulumi.get(self, "category")

    @_builtins.property
    @pulumi.getter(name="contentType")
    def content_type(self) -> pulumi.Output[Optional[_builtins.str]]:
        """
        The upload's content type (for example, application/octet-stream).
        """
        return pulumi.get(self, "content_type")

    @_builtins.property
    @pulumi.getter
    def created(self) -> pulumi.Output[_builtins.str]:
        """
        When the upload was created.
        """
        return pulumi.get(self, "created")

    @_builtins.property
    @pulumi.getter
    def name(self) -> pulumi.Output[_builtins.str]:
        """
        The upload's file name.
        """
        return pulumi.get(self, "name")

    @_builtins.property
    @pulumi.getter(name="projectArn")
    def project_arn(self) -> pulumi.Output[_builtins.str]:
        """
        The ARN of the project for the upload.
        """
        return pulumi.get(self, "project_arn")

    @_builtins.property
    @pulumi.getter
    def status(self) -> pulumi.Output['UploadStatus']:
        """
        The upload's status.
        """
        return pulumi.get(self, "status")

    @_builtins.property
    @pulumi.getter
    def type(self) -> pulumi.Output['UploadType']:
        """
        The upload's type.
        """
        return pulumi.get(self, "type")

