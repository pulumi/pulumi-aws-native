# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities
from . import outputs
from .. import _inputs as _root_inputs
from .. import outputs as _root_outputs
from ._inputs import *

__all__ = ['WorkspaceIpGroupArgs', 'WorkspaceIpGroup']

@pulumi.input_type
class WorkspaceIpGroupArgs:
    def __init__(__self__, *,
                 group_desc: pulumi.Input[Optional[_builtins.str]] = None,
                 group_name: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input['_root_inputs.TagArgs']]]] = None,
                 user_rules: pulumi.Input[Optional[Sequence[pulumi.Input['WorkspaceIpGroupIpRuleItemArgs']]]] = None):
        """
        The set of arguments for constructing a WorkspaceIpGroup resource.

        :param pulumi.Input[_builtins.str] group_desc: The description of the group.
        :param pulumi.Input[_builtins.str] group_name: The name of the group.
        :param pulumi.Input[Sequence[pulumi.Input['_root_inputs.TagArgs']]] tags: The tags for the IP access control group.
        :param pulumi.Input[Sequence[pulumi.Input['WorkspaceIpGroupIpRuleItemArgs']]] user_rules: The rules for the IP access control group.
        """
        if group_desc is not None:
            pulumi.set(__self__, "group_desc", group_desc)
        if group_name is not None:
            pulumi.set(__self__, "group_name", group_name)
        if tags is not None:
            pulumi.set(__self__, "tags", tags)
        if user_rules is not None:
            pulumi.set(__self__, "user_rules", user_rules)

    @_builtins.property
    @pulumi.getter(name="groupDesc")
    def group_desc(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        The description of the group.
        """
        return pulumi.get(self, "group_desc")

    @group_desc.setter
    def group_desc(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "group_desc", value)

    @_builtins.property
    @pulumi.getter(name="groupName")
    def group_name(self) -> pulumi.Input[Optional[_builtins.str]]:
        """
        The name of the group.
        """
        return pulumi.get(self, "group_name")

    @group_name.setter
    def group_name(self, value: pulumi.Input[Optional[_builtins.str]]):
        pulumi.set(self, "group_name", value)

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['_root_inputs.TagArgs']]]]:
        """
        The tags for the IP access control group.
        """
        return pulumi.get(self, "tags")

    @tags.setter
    def tags(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['_root_inputs.TagArgs']]]]):
        pulumi.set(self, "tags", value)

    @_builtins.property
    @pulumi.getter(name="userRules")
    def user_rules(self) -> pulumi.Input[Optional[Sequence[pulumi.Input['WorkspaceIpGroupIpRuleItemArgs']]]]:
        """
        The rules for the IP access control group.
        """
        return pulumi.get(self, "user_rules")

    @user_rules.setter
    def user_rules(self, value: pulumi.Input[Optional[Sequence[pulumi.Input['WorkspaceIpGroupIpRuleItemArgs']]]]):
        pulumi.set(self, "user_rules", value)


@pulumi.type_token("aws-native:workspaces:WorkspaceIpGroup")
class WorkspaceIpGroup(pulumi.CustomResource):
    @overload
    def __init__(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 group_desc: pulumi.Input[Optional[_builtins.str]] = None,
                 group_name: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['_root_inputs.TagArgs', '_root_inputs.TagArgsDict']]]]] = None,
                 user_rules: pulumi.Input[Optional[Sequence[pulumi.Input[Union['WorkspaceIpGroupIpRuleItemArgs', 'WorkspaceIpGroupIpRuleItemArgsDict']]]]] = None,
                 __props__=None):
        """
        Resource type definition for an IP access control group for Amazon WorkSpaces.

        :param str resource_name: The name of the resource.
        :param pulumi.ResourceOptions opts: Options for the resource.
        :param pulumi.Input[_builtins.str] group_desc: The description of the group.
        :param pulumi.Input[_builtins.str] group_name: The name of the group.
        :param pulumi.Input[Sequence[pulumi.Input[Union['_root_inputs.TagArgs', '_root_inputs.TagArgsDict']]]] tags: The tags for the IP access control group.
        :param pulumi.Input[Sequence[pulumi.Input[Union['WorkspaceIpGroupIpRuleItemArgs', 'WorkspaceIpGroupIpRuleItemArgsDict']]]] user_rules: The rules for the IP access control group.
        """
        ...
    @overload
    def __init__(__self__,
                 resource_name: str,
                 args: Optional[WorkspaceIpGroupArgs] = None,
                 opts: Optional[pulumi.ResourceOptions] = None):
        """
        Resource type definition for an IP access control group for Amazon WorkSpaces.

        :param str resource_name: The name of the resource.
        :param WorkspaceIpGroupArgs args: The arguments to use to populate this resource's properties.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        resource_args, opts = _utilities.get_resource_args_opts(WorkspaceIpGroupArgs, pulumi.ResourceOptions, *args, **kwargs)
        if resource_args is not None:
            __self__._internal_init(resource_name, opts, **resource_args.__dict__)
        else:
            __self__._internal_init(resource_name, *args, **kwargs)

    def _internal_init(__self__,
                 resource_name: str,
                 opts: Optional[pulumi.ResourceOptions] = None,
                 group_desc: pulumi.Input[Optional[_builtins.str]] = None,
                 group_name: pulumi.Input[Optional[_builtins.str]] = None,
                 tags: pulumi.Input[Optional[Sequence[pulumi.Input[Union['_root_inputs.TagArgs', '_root_inputs.TagArgsDict']]]]] = None,
                 user_rules: pulumi.Input[Optional[Sequence[pulumi.Input[Union['WorkspaceIpGroupIpRuleItemArgs', 'WorkspaceIpGroupIpRuleItemArgsDict']]]]] = None,
                 __props__=None):
        opts = pulumi.ResourceOptions.merge(_utilities.get_resource_opts_defaults(), opts)
        if not isinstance(opts, pulumi.ResourceOptions):
            raise TypeError('Expected resource options to be a ResourceOptions instance')
        if opts.id is None:
            if __props__ is not None:
                raise TypeError('__props__ is only valid when passed in combination with a valid opts.id to get an existing resource')
            __props__ = WorkspaceIpGroupArgs.__new__(WorkspaceIpGroupArgs)

            __props__.__dict__["group_desc"] = group_desc
            __props__.__dict__["group_name"] = group_name
            __props__.__dict__["tags"] = tags
            __props__.__dict__["user_rules"] = user_rules
            __props__.__dict__["arn"] = None
            __props__.__dict__["group_id"] = None
        replace_on_changes = pulumi.ResourceOptions(replace_on_changes=["groupDesc", "groupName"])
        opts = pulumi.ResourceOptions.merge(opts, replace_on_changes)
        super(WorkspaceIpGroup, __self__).__init__(
            'aws-native:workspaces:WorkspaceIpGroup',
            resource_name,
            __props__,
            opts)

    @staticmethod
    def get(resource_name: str,
            id: pulumi.Input[str],
            opts: Optional[pulumi.ResourceOptions] = None) -> 'WorkspaceIpGroup':
        """
        Get an existing WorkspaceIpGroup resource's state with the given name, id, and optional extra
        properties used to qualify the lookup.

        :param str resource_name: The unique name of the resulting resource.
        :param pulumi.Input[str] id: The unique provider ID of the resource to lookup.
        :param pulumi.ResourceOptions opts: Options for the resource.
        """
        opts = pulumi.ResourceOptions.merge(opts, pulumi.ResourceOptions(id=id))

        __props__ = WorkspaceIpGroupArgs.__new__(WorkspaceIpGroupArgs)

        __props__.__dict__["arn"] = None
        __props__.__dict__["group_desc"] = None
        __props__.__dict__["group_id"] = None
        __props__.__dict__["group_name"] = None
        __props__.__dict__["tags"] = None
        __props__.__dict__["user_rules"] = None
        return WorkspaceIpGroup(resource_name, opts=opts, __props__=__props__)

    @_builtins.property
    @pulumi.getter
    def arn(self) -> pulumi.Output[_builtins.str]:
        """
        The Amazon Resource Name (ARN) of the IP access control group.
        """
        return pulumi.get(self, "arn")

    @_builtins.property
    @pulumi.getter(name="groupDesc")
    def group_desc(self) -> pulumi.Output[Optional[_builtins.str]]:
        """
        The description of the group.
        """
        return pulumi.get(self, "group_desc")

    @_builtins.property
    @pulumi.getter(name="groupId")
    def group_id(self) -> pulumi.Output[_builtins.str]:
        """
        The identifier of the IP access control group.
        """
        return pulumi.get(self, "group_id")

    @_builtins.property
    @pulumi.getter(name="groupName")
    def group_name(self) -> pulumi.Output[_builtins.str]:
        """
        The name of the group.
        """
        return pulumi.get(self, "group_name")

    @_builtins.property
    @pulumi.getter
    def tags(self) -> pulumi.Output[Optional[Sequence['_root_outputs.Tag']]]:
        """
        The tags for the IP access control group.
        """
        return pulumi.get(self, "tags")

    @_builtins.property
    @pulumi.getter(name="userRules")
    def user_rules(self) -> pulumi.Output[Optional[Sequence['outputs.WorkspaceIpGroupIpRuleItem']]]:
        """
        The rules for the IP access control group.
        """
        return pulumi.get(self, "user_rules")

