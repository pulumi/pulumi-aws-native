...
warnings.warn("""DBSubnetGroup is not yet supported by AWS Cloud Control API, so its creation will currently fail. Please use the classic AWS provider, if possible.""", DeprecationWarning)
...
class DBSubnetGroup(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
                 ...
                 tags: Optional[pulumi.Input[Sequence[pulumi.Input[pulumi.InputType['DBSubnetGroupTagArgs']]]]] = None,
                 __props__=None):
        pulumi.log.warn("""DBSubnetGroup is deprecated: DBSubnetGroup is not yet supported by AWS Cloud Control API, so its creation will currently fail. Please use the classic AWS provider, if possible.""")
        if opts is None:
