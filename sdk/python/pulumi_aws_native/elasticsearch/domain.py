...
warnings.warn("""Domain is not yet supported by AWS Cloud Control API, so its creation will currently fail. Please use the classic AWS provider, if possible.""", DeprecationWarning)
...
class Domain(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
                 ...
                 v_pc_options: Optional[pulumi.Input[pulumi.InputType['DomainVPCOptionsArgs']]] = None,
                 __props__=None):
        pulumi.log.warn("""Domain is deprecated: Domain is not yet supported by AWS Cloud Control API, so its creation will currently fail. Please use the classic AWS provider, if possible.""")
        if opts is None:
