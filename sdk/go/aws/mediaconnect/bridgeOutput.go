// Code generated by the Pulumi SDK Generator DO NOT EDIT.
// *** WARNING: Do not edit by hand unless you're certain you know what you are doing! ***

package mediaconnect

import (
	"context"
	"reflect"

	"github.com/pkg/errors"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// Resource schema for AWS::MediaConnect::BridgeOutput
type BridgeOutputResource struct {
	pulumi.CustomResourceState

	// The Amazon Resource Number (ARN) of the bridge.
	BridgeArn pulumi.StringOutput `pulumi:"bridgeArn"`
	// The network output name.
	Name pulumi.StringOutput `pulumi:"name"`
	// The output of the bridge.
	NetworkOutput BridgeOutputBridgeNetworkOutputOutput `pulumi:"networkOutput"`
}

// NewBridgeOutputResource registers a new resource with the given unique name, arguments, and options.
func NewBridgeOutputResource(ctx *pulumi.Context,
	name string, args *BridgeOutputResourceArgs, opts ...pulumi.ResourceOption) (*BridgeOutputResource, error) {
	if args == nil {
		return nil, errors.New("missing one or more required arguments")
	}

	if args.BridgeArn == nil {
		return nil, errors.New("invalid value for required argument 'BridgeArn'")
	}
	if args.NetworkOutput == nil {
		return nil, errors.New("invalid value for required argument 'NetworkOutput'")
	}
	var resource BridgeOutputResource
	err := ctx.RegisterResource("aws-native:mediaconnect:BridgeOutput", name, args, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
}

// GetBridgeOutputResource gets an existing BridgeOutputResource resource's state with the given name, ID, and optional
// state properties that are used to uniquely qualify the lookup (nil if not required).
func GetBridgeOutputResource(ctx *pulumi.Context,
	name string, id pulumi.IDInput, state *BridgeOutputResourceState, opts ...pulumi.ResourceOption) (*BridgeOutputResource, error) {
	var resource BridgeOutputResource
	err := ctx.ReadResource("aws-native:mediaconnect:BridgeOutput", name, id, state, &resource, opts...)
	if err != nil {
		return nil, err
	}
	return &resource, nil
}

// Input properties used for looking up and filtering BridgeOutputResource resources.
type bridgeOutputResourceState struct {
}

type BridgeOutputResourceState struct {
}

func (BridgeOutputResourceState) ElementType() reflect.Type {
	return reflect.TypeOf((*bridgeOutputResourceState)(nil)).Elem()
}

type bridgeOutputResourceArgs struct {
	// The Amazon Resource Number (ARN) of the bridge.
	BridgeArn string `pulumi:"bridgeArn"`
	// The network output name.
	Name *string `pulumi:"name"`
	// The output of the bridge.
	NetworkOutput BridgeOutputBridgeNetworkOutput `pulumi:"networkOutput"`
}

// The set of arguments for constructing a BridgeOutputResource resource.
type BridgeOutputResourceArgs struct {
	// The Amazon Resource Number (ARN) of the bridge.
	BridgeArn pulumi.StringInput
	// The network output name.
	Name pulumi.StringPtrInput
	// The output of the bridge.
	NetworkOutput BridgeOutputBridgeNetworkOutputInput
}

func (BridgeOutputResourceArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*bridgeOutputResourceArgs)(nil)).Elem()
}

type BridgeOutputResourceInput interface {
	pulumi.Input

	ToBridgeOutputResourceOutput() BridgeOutputResourceOutput
	ToBridgeOutputResourceOutputWithContext(ctx context.Context) BridgeOutputResourceOutput
}

func (*BridgeOutputResource) ElementType() reflect.Type {
	return reflect.TypeOf((**BridgeOutputResource)(nil)).Elem()
}

func (i *BridgeOutputResource) ToBridgeOutputResourceOutput() BridgeOutputResourceOutput {
	return i.ToBridgeOutputResourceOutputWithContext(context.Background())
}

func (i *BridgeOutputResource) ToBridgeOutputResourceOutputWithContext(ctx context.Context) BridgeOutputResourceOutput {
	return pulumi.ToOutputWithContext(ctx, i).(BridgeOutputResourceOutput)
}

type BridgeOutputResourceOutput struct{ *pulumi.OutputState }

func (BridgeOutputResourceOutput) ElementType() reflect.Type {
	return reflect.TypeOf((**BridgeOutputResource)(nil)).Elem()
}

func (o BridgeOutputResourceOutput) ToBridgeOutputResourceOutput() BridgeOutputResourceOutput {
	return o
}

func (o BridgeOutputResourceOutput) ToBridgeOutputResourceOutputWithContext(ctx context.Context) BridgeOutputResourceOutput {
	return o
}

// The Amazon Resource Number (ARN) of the bridge.
func (o BridgeOutputResourceOutput) BridgeArn() pulumi.StringOutput {
	return o.ApplyT(func(v *BridgeOutputResource) pulumi.StringOutput { return v.BridgeArn }).(pulumi.StringOutput)
}

// The network output name.
func (o BridgeOutputResourceOutput) Name() pulumi.StringOutput {
	return o.ApplyT(func(v *BridgeOutputResource) pulumi.StringOutput { return v.Name }).(pulumi.StringOutput)
}

// The output of the bridge.
func (o BridgeOutputResourceOutput) NetworkOutput() BridgeOutputBridgeNetworkOutputOutput {
	return o.ApplyT(func(v *BridgeOutputResource) BridgeOutputBridgeNetworkOutputOutput { return v.NetworkOutput }).(BridgeOutputBridgeNetworkOutputOutput)
}

func init() {
	pulumi.RegisterInputType(reflect.TypeOf((*BridgeOutputResourceInput)(nil)).Elem(), &BridgeOutputResource{})
	pulumi.RegisterOutputType(BridgeOutputResourceOutput{})
}
