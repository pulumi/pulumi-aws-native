// Code generated by the Pulumi SDK Generator DO NOT EDIT.
// *** WARNING: Do not edit by hand unless you're certain you know what you are doing! ***

package waf

import (
	"context"
	"reflect"

	"github.com/pulumi/pulumi-aws-native/sdk/go/aws/internal"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumix"
)

// Resource Type definition for AWS::WAF::ByteMatchSet
func LookupByteMatchSet(ctx *pulumi.Context, args *LookupByteMatchSetArgs, opts ...pulumi.InvokeOption) (*LookupByteMatchSetResult, error) {
	opts = internal.PkgInvokeDefaultOpts(opts)
	var rv LookupByteMatchSetResult
	err := ctx.Invoke("aws-native:waf:getByteMatchSet", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
}

type LookupByteMatchSetArgs struct {
	Id string `pulumi:"id"`
}

type LookupByteMatchSetResult struct {
	ByteMatchTuples []ByteMatchSetByteMatchTuple `pulumi:"byteMatchTuples"`
	Id              *string                      `pulumi:"id"`
}

func LookupByteMatchSetOutput(ctx *pulumi.Context, args LookupByteMatchSetOutputArgs, opts ...pulumi.InvokeOption) LookupByteMatchSetResultOutput {
	return pulumi.ToOutputWithContext(context.Background(), args).
		ApplyT(func(v interface{}) (LookupByteMatchSetResult, error) {
			args := v.(LookupByteMatchSetArgs)
			r, err := LookupByteMatchSet(ctx, &args, opts...)
			var s LookupByteMatchSetResult
			if r != nil {
				s = *r
			}
			return s, err
		}).(LookupByteMatchSetResultOutput)
}

type LookupByteMatchSetOutputArgs struct {
	Id pulumi.StringInput `pulumi:"id"`
}

func (LookupByteMatchSetOutputArgs) ElementType() reflect.Type {
	return reflect.TypeOf((*LookupByteMatchSetArgs)(nil)).Elem()
}

type LookupByteMatchSetResultOutput struct{ *pulumi.OutputState }

func (LookupByteMatchSetResultOutput) ElementType() reflect.Type {
	return reflect.TypeOf((*LookupByteMatchSetResult)(nil)).Elem()
}

func (o LookupByteMatchSetResultOutput) ToLookupByteMatchSetResultOutput() LookupByteMatchSetResultOutput {
	return o
}

func (o LookupByteMatchSetResultOutput) ToLookupByteMatchSetResultOutputWithContext(ctx context.Context) LookupByteMatchSetResultOutput {
	return o
}

func (o LookupByteMatchSetResultOutput) ToOutput(ctx context.Context) pulumix.Output[LookupByteMatchSetResult] {
	return pulumix.Output[LookupByteMatchSetResult]{
		OutputState: o.OutputState,
	}
}

func (o LookupByteMatchSetResultOutput) ByteMatchTuples() ByteMatchSetByteMatchTupleArrayOutput {
	return o.ApplyT(func(v LookupByteMatchSetResult) []ByteMatchSetByteMatchTuple { return v.ByteMatchTuples }).(ByteMatchSetByteMatchTupleArrayOutput)
}

func (o LookupByteMatchSetResultOutput) Id() pulumi.StringPtrOutput {
	return o.ApplyT(func(v LookupByteMatchSetResult) *string { return v.Id }).(pulumi.StringPtrOutput)
}

func init() {
	pulumi.RegisterOutputType(LookupByteMatchSetResultOutput{})
}
