// *** WARNING: this file was generated by pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AwsNative.AppFlow
{
    /// <summary>
    /// Resource schema for AWS::AppFlow::Connector
    /// </summary>
    [AwsNativeResourceType("aws-native:appflow:Connector")]
    public partial class Connector : global::Pulumi.CustomResource
    {
        /// <summary>
        ///  The arn of the connector. The arn is unique for each ConnectorRegistration in your AWS account.
        /// </summary>
        [Output("connectorArn")]
        public Output<string> ConnectorArn { get; private set; } = null!;

        /// <summary>
        ///  The name of the connector. The name is unique for each ConnectorRegistration in your AWS account.
        /// </summary>
        [Output("connectorLabel")]
        public Output<string?> ConnectorLabel { get; private set; } = null!;

        /// <summary>
        /// Contains information about the configuration of the connector being registered.
        /// </summary>
        [Output("connectorProvisioningConfig")]
        public Output<Outputs.ConnectorProvisioningConfig> ConnectorProvisioningConfig { get; private set; } = null!;

        /// <summary>
        /// The provisioning type of the connector. Currently the only supported value is LAMBDA. 
        /// </summary>
        [Output("connectorProvisioningType")]
        public Output<string> ConnectorProvisioningType { get; private set; } = null!;

        /// <summary>
        /// A description about the connector that's being registered.
        /// </summary>
        [Output("description")]
        public Output<string?> Description { get; private set; } = null!;


        /// <summary>
        /// Create a Connector resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public Connector(string name, ConnectorArgs args, CustomResourceOptions? options = null)
            : base("aws-native:appflow:Connector", name, args ?? new ConnectorArgs(), MakeResourceOptions(options, ""))
        {
        }

        private Connector(string name, Input<string> id, CustomResourceOptions? options = null)
            : base("aws-native:appflow:Connector", name, null, MakeResourceOptions(options, id))
        {
        }

        private static CustomResourceOptions MakeResourceOptions(CustomResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new CustomResourceOptions
            {
                Version = Utilities.Version,
                ReplaceOnChanges =
                {
                    "connectorLabel",
                },
            };
            var merged = CustomResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
        /// <summary>
        /// Get an existing Connector resource's state with the given name, ID, and optional extra
        /// properties used to qualify the lookup.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resulting resource.</param>
        /// <param name="id">The unique provider ID of the resource to lookup.</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public static Connector Get(string name, Input<string> id, CustomResourceOptions? options = null)
        {
            return new Connector(name, id, options);
        }
    }

    public sealed class ConnectorArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        ///  The name of the connector. The name is unique for each ConnectorRegistration in your AWS account.
        /// </summary>
        [Input("connectorLabel")]
        public Input<string>? ConnectorLabel { get; set; }

        /// <summary>
        /// Contains information about the configuration of the connector being registered.
        /// </summary>
        [Input("connectorProvisioningConfig", required: true)]
        public Input<Inputs.ConnectorProvisioningConfigArgs> ConnectorProvisioningConfig { get; set; } = null!;

        /// <summary>
        /// The provisioning type of the connector. Currently the only supported value is LAMBDA. 
        /// </summary>
        [Input("connectorProvisioningType", required: true)]
        public Input<string> ConnectorProvisioningType { get; set; } = null!;

        /// <summary>
        /// A description about the connector that's being registered.
        /// </summary>
        [Input("description")]
        public Input<string>? Description { get; set; }

        public ConnectorArgs()
        {
        }
        public static new ConnectorArgs Empty => new ConnectorArgs();
    }
}
