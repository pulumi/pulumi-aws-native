...
// GlobalTransformCache is the singleton cache for compiled transforms.
// TODO: Consider refactoring to dependency injection for better test isolation and
// bounded memory growth. The current singleton pattern works for per-stack provider
// lifecycles but may grow unbounded in long-running scenarios.
...
// ValuesEquivalent compares two values, handling type differences and regex patterns.
func ValuesEquivalent(original, transformedNew interface{}) bool {
	...
	// Check if transformedNew is a regex pattern (wrapped in quotes in some CF transforms)
