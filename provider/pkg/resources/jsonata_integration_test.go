	...
	t.Run("suppresses case-only differences in identifiers", func(t *testing.T) {
		diff := &resource.ObjectDiff{
			Updates: map[resource.PropertyKey]resource.ValueDiff{
				"dbClusterIdentifier": {
					Old: resource.NewStringProperty("MY-CLUSTER"),
					New: resource.NewStringProperty("my-cluster"),
				},
				"engine": {
					Old: resource.NewStringProperty("AURORA-MYSQL"),
					New: resource.NewStringProperty("aurora-mysql"),
				},
			},
		}
		...
		result := suppressPropertyTransformDiffs(resourceToken, spec, diff, originalInputs)
	...
	t.Run("preserves real value changes", func(t *testing.T) {
		diff := &resource.ObjectDiff{
			Updates: map[resource.PropertyKey]resource.ValueDiff{
				"engine": {
					Old: resource.NewStringProperty("aurora-mysql"),
					New: resource.NewStringProperty("aurora-postgresql"),
				},
			},
		}
...
func TestIntegration_ComplexLookupTransform(t *testing.T) {
	...
	t.Run("lookup returns nil for unknown value", func(t *testing.T) {
...
func TestIntegration_NoTransformsNoEffect(t *testing.T) {
	...
	diff := &resource.ObjectDiff{
		Updates: map[resource.PropertyKey]resource.ValueDiff{
			"bucketName": {
				Old: resource.NewStringProperty("old-bucket"),
				New: resource.NewStringProperty("new-bucket"),
			},
		},
	}
