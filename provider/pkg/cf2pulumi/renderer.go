...
// detectPseudoParameters detects uses of pseudo-parameters in a value.
func (ctx *renderContext) detectPseudoParameters(node ast.Node) {
	switch node := node.(type) {
	case *ast.SequenceNode:
		for _, v := range node.Values {
			ctx.detectPseudoParameters(v)
	...
	case *ast.MappingNode, *ast.MappingValueNode:
		...
		if len(values) == 1 && functions.Has(keyString(values[0])) {
			...
			for _, v := range values {
				ctx.detectPseudoParameters(v.Value)
			}
		}
	case *ast.TagNode:
		if fnName, ok := functionTags[node.Start.Value]; ok {
			ctx.detectFunctionPseudoParameters(fnName, node.Value)
		}
	}
}
